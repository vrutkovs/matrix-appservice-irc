...
module.exports.onPrivateMessage = function (server, fromUser, toUser, action) {
    ...
    var bridgedIrcClient = pool.getBridgedClientByNick(toUser.server, toUser.nick);
    if (!bridgedIrcClient) {
    ...
    matrixLib.ircToMatrixUser(fromUser).then(function (user) {
        ...
        req.log.info("Mapped to %s", JSON.stringify(user));
        if (!server.allowsPms()) {
            req.log.error("Server %s disallows PMs.", server.domain);
            return;
        }
        store.rooms.getMatrixPmRoom(bridgedIrcClient.userId, virtualMatrixUser.userId).done(function (pmRoom) {
            if (pmRoom) {
                req.log.info("Relaying PM in room %s", pmRoom.roomId);
                req.mxLib.sendAction(pmRoom, virtualMatrixUser, mxAction).done(req.sucFn, req.errFn);
                return;
            }
            // make a pm room then send the message
            req.log.info("Creating a PM room with %s", bridgedIrcClient.userId);
            req.mxLib.createRoomWithUser(virtualMatrixUser.userId, bridgedIrcClient.userId, fromUser.nick + " (PM on " + server.domain + ")").done(function (mxRoom) {
                // the nick is the channel
                var ircRoom = new IrcRoom(server, fromUser.nick);
                store.rooms.setPmRoom(ircRoom, mxRoom, bridgedIrcClient.userId, virtualMatrixUser.userId).then(function () {
                    return req.mxLib.sendAction(mxRoom, virtualMatrixUser, mxAction);
...
module.exports.onMessage = function (server, fromUser, channel, action) {
    ...
    matrixLib.ircToMatrixUser(fromUser).then(function (user) {
        ...
        store.rooms.getMatrixRoomsForChannel(server, channel).then(function (matrixRooms) {
            var promises = [];
            matrixRooms.forEach(function (room) {
                req.log.info("Relaying in room %s", room.roomId);
                promises.push(req.mxLib.sendAction(room, virtualMatrixUser, mxAction));
            });
...
module.exports.onJoin = function (server, joiningUser, chan, kind) {
    ...
    matrixLib.ircToMatrixUser(joiningUser).then(function (user) {
        req.log.info("Mapped nick %s to %s", nick, JSON.stringify(user));
        matrixUser = user;
        ...
        matrixRooms.forEach(function (room) {
            ...
            promises.push(req.mxLib.joinRoom(room.roomId, matrixUser));
        });
        if (matrixRooms.length === 0) {
...
module.exports.onPart = function (server, leavingUser, chan, kind) {
    ...
    matrixLib.ircToMatrixUser(leavingUser).then(function (user) {
        ...
        matrixRooms.forEach(function (room) {
            ...
            promises.push(req.mxLib.leaveRoom(matrixUser.userId, room.roomId));
        });
        if (matrixRooms.length === 0) {
            req.log.info("No mapped matrix rooms for IRC channel %s", chan);
        }
...
module.exports.onMode = function (server, channel, by, mode, enabled, arg) {
    ...
    if (["k", "i"].indexOf(mode) !== -1) {
        ...
        store.rooms.getMatrixRoomsForChannel(server, channel).then(function (matrixRooms) {
            var promises = [];
            matrixRooms.forEach(function (room) {
                req.log.info(enabled ? "Locking room %s" : "Reverting %s back to default join_rule", room.roomId);
                if (enabled) {
                    promises.push(req.mxLib.setInviteOnly(room, true));
                } else {
                    // don't "unlock"; the room may have been invite
                    // only from the beginning.
                    enabled = server.getJoinRule() === "invite";
                    promises.push(req.mxLib.setInviteOnly(room, enabled));
                }
            });
            if (matrixRooms.length === 0) {
                req.log.info("No mapped matrix rooms for IRC channel %s", channel);
