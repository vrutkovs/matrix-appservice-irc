...
var createMatrixUserForIrcUser = function createMatrixUserForIrcUser(ircUser, req) {
    var defer = q.defer();
    ...
    matrixLib.ircToMatrixUser(ircUser).then(function (user) {
        ...
        req.log.error("Virtual user creation for %s failed: %s", ircUser.nick, err);
        defer.reject(err);
    })["catch"](log.logErr);
 ...
 */
var holdEvent = function holdEvent(event, request) {
    processingInvitesForRooms[event.room_id + event.user_id]["finally"](function () {
        request.log.info("Finished holding event for %s in room %s", event.user_id, event.room_id);
 ...
 */
var handleAdminRoomInvite = function handleAdminRoomInvite(event, req, inviter, botUser) {
 ...
 */
var handleInviteFromBot = function handleInviteFromBot(event, req, invitedIrcUser) {
    req.log.info("Handling invite from bot directed at %s on %s", invitedIrcUser.server.domain, invitedIrcUser.nick);
...
var handleInviteFromUser = function handleInviteFromUser(event, req, invitedIrcUser) {
    req.log.info("Handling invite from user directed at %s on %s", invitedIrcUser.server.domain, invitedIrcUser.nick);
    ...
    createMatrixUserForIrcUser(invitedIrcUser, req).then(function (mxUser) {
        ...
        return req.mxLib.isPmRoom(invitedUser.userId, event.room_id, event.user_id);
...
// === Admin room handling ===
var onAdminMessage = function onAdminMessage(event, req, adminRoom) {
    req.log.info("Received admin message from %s", event.user_id);
    var botUser = new MatrixUser(matrixLib.getAppServiceUserId(), null, false);
    ...
    if (event.content.body.indexOf("!nick") === 0) {
        ...
        if (!ircServer || !nick) {
            ...
            if (clientList.length === 0) {
                connectedNetworksStr = "You are not currently connected to any " + "IRC networks which have nick changes enabled.";
            } else {
                connectedNetworksStr = "Currently connected to IRC networks:\n";
                for (i = 0; i < clientList.length; i++) {
                    connectedNetworksStr += clientList[i].server.domain + " as " + clientList[i].nick + "\n";
                }
            }
        ...
        } else if (server.hasInviteRooms() && !server.isInWhitelist(event.user_id)) {
        ...
        store.rooms.getMatrixRoomsForChannel(server, ircChannel).done(function (matrixRooms) {
            if (matrixRooms.length > 0) {
                // already tracking channel, so just invite them.
                var promises = [];
                matrixRooms.forEach(function (room) {
                    req.log.info("Inviting %s to room %s", event.user_id, room.roomId);
                ...
                ircLib.trackChannel(server, ircChannel).then(function (room) {
                    ircRoom = room;
                    // implied private
                    return req.mxLib.createRoomWithUser(undefined, event.user_id, ircChannel);
                }).then(function (mxRoom) {
                    return store.rooms.set(ircRoom, mxRoom);
                }).then(function () {
                    req.log.info("Created a room to track %s on %s and invited %s", ircRoom.channel, server.domain, event.user_id);
                    req.sucFn();
                })["catch"](req.errFn);
            }
        }, req.errFn);
...
module.exports.onInvite = function (event, inviter, invitee) {
    ...
    // invite yet!)
    processingInvitesForRooms[event.room_id + event.state_key] = req.defer.promise;
...
module.exports.onJoin = function (event, user) {
    ...
    store.rooms.getIrcChannelsForRoomId(event.room_id).done(function (ircRooms) {
        // =========== Bridge Bot Joining ===========
        ...
        ircRooms = ircRooms.filter(function (room) {
            return room.server.shouldSyncMembershipToIrc(syncKind, event.room_id);
        });

        if (ircRooms.length === 0) {
            req.log.info("No tracked channels which mirror joins for this room.");
            req.sucFn();
            return;
        }
        ...
        ircRooms.forEach(function (room) {
            if (room.server.claimsUserId(user.userId)) {
                req.log.info("%s is a virtual user (claimed by %s)", user.userId, room.server.domain);
                return;
            }
            // get the virtual IRC user for this user
            promises.push(req.ircLib.getBridgedClient(room.server, user.userId).then(function (bridgedClient) {
                // join each channel
...
module.exports.onLeave = function (event, user) {
    ...
    clientList = clientList.filter(function (client) {
        return client.server.shouldSyncMembershipToIrc(syncKind, event.room_id) && !client.server.claimsUserId(user.userId); // not a virtual user
    });
...
module.exports.onMessage = function (event, existingRequest) {
    ...
    req.log.info("%s usr=%s rm=%s body=%s", event.type, event.user_id, event.room_id, event.content.body ? event.content.body.substring(0, 20) : "");

    if (processingInvitesForRooms[event.room_id + event.user_id]) {
        req.log.info("Holding request for %s until invite for room %s is done.", event.user_id, event.room_id);
        holdEvent(event, req);
        return req.defer.promise;
    }
    ...
    store.rooms.getIrcChannelsForRoomId(event.room_id).done(function (ircRooms) {
        if (ircRooms.length === 0) {
            ...
            store.rooms.getAdminRoomById(event.room_id).done(function (room) {
                if (!room) {
        ...
        ircRooms.forEach(function (ircRoom) {
            if (ircRoom.server.claimsUserId(event.user_id)) {
                req.log.info("%s is a virtual user (claimed by %s)", event.user_id, ircRoom.server.domain);
                return;
            }
            req.log.info("Relaying message in %s on %s", ircRoom.channel, ircRoom.server.domain);
            ...
            if (!ircLib.getIrcUserFromCache(ircRoom.server, event.user_id)) {
                promises.push(req.mxLib.getDisplayName(event.room_id, event.user_id).then(function (displayName) {
                    return req.ircLib.getBridgedClient(ircRoom.server, event.user_id, displayName);
                }, function (err) {
                    req.log.error("Failed to get display name: %s", err);
                    // this is non-fatal, continue.
                    return req.ircLib.getBridgedClient(ircRoom.server, event.user_id);
                }).then(function (ircUser) {
                    return req.ircLib.sendAction(ircRoom, ircUser, ircAction);
                }));
            } else {
                promises.push(req.ircLib.getBridgedClient(ircRoom.server, event.user_id).then(function (ircUser) {
                    return req.ircLib.sendAction(ircRoom, ircUser, ircAction);
                }));
            }
        });
...
module.exports.onAliasQuery = function (roomAlias) {
    ...
    req.log.info("Mapped to %s on %s", channelInfo.channel, channelInfo.server.domain);

    // See if we are already tracking this channel (case-insensitive
    // channels but case-sensitive aliases)
    store.rooms.getMatrixRoomsForChannel(channelInfo.server, channelInfo.channel).done(function (matrixRooms) {
        if (matrixRooms.length === 0) {
            ...
            ircLib.trackChannel(channelInfo.server, channelInfo.channel).then(function (ircRoom) {
                req.log.info("Bot is now tracking channel.");
                return req.mxLib.createRoomWithAlias(roomAlias, channelInfo.channel, undefined, channelInfo.server.getJoinRule(), channelInfo.server.shouldPublishRooms());
                ...
                var ircRoom = new IrcRoom(channelInfo.server, channelInfo.channel);
            ...
            req.mxLib.addAlias(roomId, roomAlias).done(req.sucFn, req.errFn);
        }
    }, req.errFn);
