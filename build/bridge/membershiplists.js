...
module.exports.getChannelsToJoin = function (server) {
    ...
    getSyncableRooms(server).then(function (rooms) {
        ...
        rooms.forEach(function (room) {
            promises.push(store.rooms.getIrcChannelsForRoomId(room.roomId).then(function (ircRooms) {
                ...
                ircRooms.forEach(function (ircRoom) {
                    ...
                    log.debug("%s should be joined because %s real Matrix users are in room %s", ircRoom.channel, room.reals.length, room.roomId);
        ...
        q.allSettled(promises).then(function () {
            ...
            log.debug("getChannelsToJoin => %s should be synced: %s", chans.length, JSON.stringify(chans));
            defer.resolve(chans);
        });
    });
...
module.exports.checkBotPartRoom = function (ircRoom, req) {
    ...
    store.rooms.getMatrixRoomsForChannel(ircRoom.server, ircRoom.channel).done(function (matrixRooms) {
        if (matrixRooms.length === 0) {
            ...
            mxLib.roomState(roomId).done(function (res) {
                ...
                log.debug("%s Matrix users are in room %s", data.reals.length, roomId);
        ...
        log.error("Cannot get matrix rooms for channel %s: %s", ircRoom.channel, err);
    });
};
...
function joinMatrixUsersToChannels(rooms, server) {
    ...
    rooms.forEach(function (r) {
        if (!server.shouldSyncMembershipToIrc("initial", r.roomId)) {
            log.debug("Skipping room %s according to config rules (matrixToIrc=false)", r.roomId);
            return;
        }
    ...
    function joinNextUser() {
        ...
        log.debug("Injecting join event for %s in %s (%s left)", entry.userId, entry.roomId, entries.length);
