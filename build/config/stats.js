...
var sendStat = function sendStat(metricName, value, type) {
    if (!endpoint) {
        return;
    }
    var msg = new Buffer(metricName + ":" + value + "|" + type);
    client.send(msg, 0, msg.length, endpoint.port, endpoint.hostname, function (err) {
        if (err) {
            log.error(err);
            if (err.stack) {
                log.error(err.stack);
...
var sendMemoryUsage = function sendMemoryUsage() {
    setTimeout(function () {
...
module.exports.ircClients = function (domain, numConnectedClients) {
    ...
    sendStat("ircas." + jobName + ".irc.clients.connected." + domain.replace(/\./g, "_"), numConnectedClients, "g");
};
...
module.exports.request = function (isFromIrc, outcome, durationMs) {
    ...
    sendStat("ircas." + jobName + ".req." + direction + "." + outcome, durationMs, "ms");
};