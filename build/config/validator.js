    ...
    if (errors.length > 0) {
        errors.forEach(function (error) {
            ...
            if (error.constraintName == "pattern") {
                log.error("The key %s has the value %s which fails to pass the " + "regex check: %s", error.instanceContext, error.testedValue, error.constraintValue);
            }
        });
        return null;
    }
...
function createNewConfig(cfg) {
    var newConfig = {};
    newConfig.appService = cfg.appService;
    newConfig.appService.localpart = newConfig.appService.localpart || "appservice-irc";
    newConfig.databaseUri = cfg.ircService.databaseUri;
    newConfig.statsd = cfg.ircService.statsd || {};

    var defaultIdent = {
        enabled: false,
        port: 113
    };
    newConfig.ident = extend(true, defaultIdent, cfg.ircService.ident);

    var defaultLogging = {
        level: "debug",
        toConsole: true
    };
    newConfig.logging = cfg.ircService.logging || defaultLogging;
    ...
    Object.keys(cfg.ircService.servers).forEach(function (serverName) {
        ...
        if (server.dynamicChannels.visibility) {
            ...
            log.warn("[DEPRECATED] Use of the config field dynamicChannels.visibility" + " is deprecated and will produce an error in a later release." + " Use dynamicChannels.published, dynamicChannels.joinRule and" + " dynamicChannels.createAlias instead.");
        ...
        newConfig.servers[serverName] = extend(true, defaultServerConfig, server);
    });
    return newConfig;
}