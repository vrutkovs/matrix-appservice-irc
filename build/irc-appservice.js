...
var prepareDatabase = function prepareDatabase(config) {
    if (dbConnPromise) {
    ...
    dbConnPromise = store.connectToDatabase(config.databaseUri).then(function () {
        ...
        Object.keys(config.servers).forEach(function (domain) {
            ...
            // set new mappings
            promises.push(store.setServerFromConfig(server, config.servers[domain]));
    ...
    return prepareDatabase(config).then(function () {
        ...
        for (i = 0; i < globalServers.length; i++) {
            ...
            if (server.createsDynamicAliases()) {
                registration.addRegexPattern("aliases", server.getAliasRegex(), true);
            }
            registration.addRegexPattern("users", server.getUserRegex(), true);
        }
...
module.exports.runService = function (appService, config, skipCrcCheck) {
    ...
    return prepareDatabase(config).then(function () {
        ...
        if (!dbToken) {
            throw new Error("No stored homeserver token. Did you run --generate-registration ?");
        }
        if (!skipCrcCheck && !module.exports.isValidHomeserverToken(dbToken, config)) {
            throw new Error("Token " + dbToken + " failed the CRC - You have updated the " + "config file without calling --generate-registration.\n" + "Either skip this check (by adding -s) or generate a registration.");
        }
