...
module.exports = {
     ...
     */
    create: function create(server, opts, onCreatedCallback) {
        if (!opts.nick || !server) {
        ...
        var d = q.defer();
        var returnClient = function returnClient(cli) {
            d.resolve(cli);
        };
        var retryConnection = function retryConnection(e) {
            var nodeClient = new irc.Client(server.domain, opts.nick, connectionOpts);
            var inst = new ConnectionInstance(nodeClient, server.domain, opts.nick);
