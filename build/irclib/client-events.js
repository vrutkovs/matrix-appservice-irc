...
var CLEANUP_TIME_MS = 1000 * 60 * 1; // 10min
...
var processed = new ProcessedDict();
processed.startCleaner();

/*
 * Attempt to claim this message as this client
 * @return {boolean} True if you successfully claimed it.
 */
var attemptClaim = function attemptClaim(client, msg) {
    ...
    var handledByNick = processed.getClaimer(domain, hash);
    // we claim it if no one else has or if we previously did this hash.
    var shouldClaim = handledByNick === null || handledByNick === client.nick;
    if (shouldClaim) {
        log.debug("%s is claiming a hash for cmd %s", client.nick, msg.rawCommand);
        processed.claim(domain, hash, client.nick, msg.rawCommand);
        return true;
        ...
        if (!owner) {
            // finders keepers
            log.debug("%s is stealing hash %s from %s because they are dead", client.nick, hash, handledByNick);
            processed.claim(domain, hash, client.nick, msg.rawCommand);
            return true;
        }
    }
...
var hookIfClaimed = function hookIfClaimed(client, connInst, eventName, fn) {
...
module.exports.addHooks = function (client, connInst, callbacks) {
    ...
    var createUser = function createUser(nick) {
        return new IrcUser(server, nick, pool.getBridgedClientByNick(server, nick) !== undefined);
    };
    ...
    if (!client.isBot) {
        ...
        connInst.addListener("message", function (from, to, text) {
            ...
            callbacks.onPrivateMessage(server, createUser(from), createUser(to), actions.irc.createMessage(text));
        });
        connInst.addListener("notice", function (from, to, text) {
            if (!from || to.indexOf("#") === 0) {
                return;
            }
            callbacks.onPrivateMessage(server, createUser(from), createUser(to), actions.irc.createNotice(text));
        });
        connInst.addListener("ctcp-privmsg", function (from, to, text) {
            ...
            if (text.indexOf("ACTION ") === 0) {
                callbacks.onPrivateMessage(server, createUser(from), createUser(to), actions.irc.createEmote(text.substring("ACTION ".length)));
            }
        });
    }
    ...
    var purgeNames = function purgeNames() {
        var promise = popName();
        if (promise) {
    ...
    hookIfClaimed(client, connInst, "names", function (chan, names) {
        if (names) {
            ...
            client.log.info("NAMEs: Adding %s nicks from %s.", userlist.length, chan);
    ...
    hookIfClaimed(client, connInst, "message", function (from, to, text) {
        ...
        callbacks.onMessage(server, createUser(from), to, actions.irc.createMessage(text));
    });
    hookIfClaimed(client, connInst, "ctcp-privmsg", function (from, to, text) {
        ...
        if (text.indexOf("ACTION ") === 0) {
            callbacks.onMessage(server, createUser(from), to, actions.irc.createEmote(text.substring("ACTION ".length)));
        }
    });
    hookIfClaimed(client, connInst, "notice", function (from, to, text) {
        ...
        if (from) {
            ...
            callbacks.onMessage(server, createUser(from), to, actions.irc.createNotice(text));
        }
    });
    hookIfClaimed(client, connInst, "topic", function (channel, topic, nick) {
        ...
        callbacks.onMessage(server, createUser(nick), channel, actions.irc.createTopic(topic));
    });
};