...
module.exports.addBot = function (server, client) {
    if (botClients[server.domain]) {
        log.error("Bot for %s already exists (old=%s new=%s) - disconnecting it.", server.domain, botClients[server.domain]._id, client._id);
...
function checkClientLimit(server) {
    ...
    oldest.disconnect("Client limit exceeded: " + server.getMaxClients()).done(function () {
        log.info("Client limit exceeded: Disconnected %s on %s.", oldest.nick, oldest.server.domain);
    }, function (e) {
        log.error("Error when disconnecting %s on server %s: %s", oldest.nick, oldest.server.domain, JSON.stringify(e));
    });
}
...
var onClientDisconnected = function onClientDisconnected(virtualUser) {
    ...
    // Reconnect this user
    log.debug("onClientDisconnected: <%s> Reconnecting %s@%s in %sms", virtualUser._id, virtualUser.nick, virtualUser.server.domain, RECONNECT_TIME_MS);
    var cli = module.exports.createIrcClient(virtualUser.ircUser, virtualUser.matrixUser, virtualUser.isBot);
    ...
    setTimeout(function () {
        cli.connect(callbacks).then(function () {
            log.info("<%s> Reconnected %s@%s", cli._id, cli.nick, cli.server.domain);
