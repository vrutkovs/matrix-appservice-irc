...
BridgedClient.prototype.connect = function (callbacks) {
    ...
    names.getIrcNames(this.ircUser, this.matrixUser).then(function (nameInfo) {
        self.log.info("Connecting to IRC server %s as %s (user=%s)", server.domain, nameInfo.nick, nameInfo.username);
        ...
        connInst.client.addListener("registered", function () {
            ...
            if (oldNick !== self.unsafeClient.nick) {
                self.log.info("REGISTERED: Nick changed from '" + oldNick + "' to '" + self.unsafeClient.nick + "'.");
        ...
        connInst.client.addListener("nick", function (old, newNick) {
            if (old === self.nick) {
                self.log.info("NICK: Nick changed from '" + old + "' to '" + newNick + "'.");
                self.nick = newNick;
...
BridgedClient.prototype.changeNick = function (newNick) {
    ...
    if (this.unsafeClient.supported && typeof this.unsafeClient.supported.nicklength == "number") {
...
BridgedClient.prototype._keepAlive = function () {
    ...
    if (idleTimeout > 0) {
        ...
        this.log.debug("_keepAlive; Restarting %ss idle timeout", idleTimeout);
        // restart the timeout
        var self = this;
        this._idleTimeout = setTimeout(function () {
            self.log.info("Idle timeout has expired");
            if (self.server.shouldSyncMembershipToIrc("initial")) {
                self.log.info("Not disconnecting because %s is mirroring matrix membership lists", self.server.domain);
                return;
            }
            if (self.isBot) {
            ...
            self.disconnect("Idle timeout reached: " + idleTimeout + "s").done(function () {
