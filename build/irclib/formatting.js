...
for (i = 0; i < htmlNames.length; i++) {
    ...
    for (j = 0; j < htmlNamesToColorCodes[htmlName].length; j++) {
...
module.exports.htmlToIrc = function (html) {
    ...
    var replacements = [[/<b>/g, STYLE_BOLD], [/<u>/g, STYLE_UNDERLINE], [/<i>/g, STYLE_ITALICS], [/<strong>/g, STYLE_BOLD]];
    Object.keys(htmlNamesToColorCodes).forEach(function (htmlColor) {
        replacements.push([new RegExp('<font color="' + htmlColor + '">', 'g'), htmlNamesToColorCodes[htmlColor][0]]);
    });
    ...
    for (i = 0; i < cleanHtml.length; i++) {
        ...
        } else if (!colorCodesToHtmlNames[ch] && ch === "<") {
            if (cleanHtml.indexOf("</font>", i) === i) {
                replacement = RESET_CODE + openStyleCodes.join("");
                cleanHtml = cleanHtml.replace("</font>", replacement);
                i += replacement.length - 1;
            } else {
                for (j = 0; j < closeTags.length; j++) {
                    ...
                    if (cleanHtml.indexOf(closeTag, i) === i) {
                        ...
                        openStyleCodes.splice(openStyleCodes.indexOf(closeTagsToStyle[closeTag]), 1);
                        replacement = RESET_CODE + openStyleCodes.join("");
                        cleanHtml = cleanHtml.replace(closeTag, replacement);
                        i += replacement.length - 1;
                    }
                }
            }
...
module.exports.ircToHtml = function (text) {
    ...
    if (colorRegex.test(text)) {
        ...
        while (groups) {
            ...
            if (fontColor) {
                text = text.replace(groups[0], '<font color="' + fontColor + '">' + coloredText + '</font>');
            } else {
                // unknown font colour
    ...
    var styleCodes = [[/\x02(.*?\x0f|[^\x02]+)(\x02)?/, ["<b>", "</b>"]], [/\x1f(.*?\x0f|[^\x1f]+)(\x1f)?/, ["<u>", "</u>"]], [/\x1d(.*?\x0f|[^\x1d]+)(\x1d)?/, ["<i>", "</i>"]]];
    for (i = 0; i < styleCodes.length; i++) {
        ...
        if (styleRegex.test(text)) {
            ...
            while (groups) {
                text = text.replace(groups[0], styleTags[0] + groups[1] + styleTags[1]);
                groups = styleRegex.exec(text);
            }
        }
    }
