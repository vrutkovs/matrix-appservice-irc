...
var servers = [];
var globalHooks = {
    onMessage: function onMessage(server, from, to, action) {},
    onJoin: function onJoin(server, nick, chan, kind) {},
    onPart: function onPart(server, nick, chan, kind) {},
    onMode: function onMode(server, channel, by, mode, enabled, arg) {}
};
...
IrcLib.prototype.partBot = function (ircRoom) {
    this.log.info("Parting bot from %s on %s", ircRoom.channel, ircRoom.server.domain);
    return getBotClient(ircRoom.server).then(function (client) {
...
IrcLib.prototype.getBridgedClient = function (server, userId, displayName) {
    ...
    lg.debug("Creating virtual irc user with nick %s for %s (display name %s)", nick, userId, displayName);
    ...
    store.ircClients.get(userId, server.domain).then(function (storedIrcUser) {
        ...
        bridgedClient.connect(globalHooks).done(function () {
            ...
            lg.error("Couldn't connect virtual user %s to %s : %s", nick, server.domain, JSON.stringify(err));
            defer.reject(err);
        });
    });
...
IrcLib.prototype.sendAction = function (ircRoom, bridgedClient, action) {
    this.log.info("Sending msg in %s as %s", ircRoom.channel, bridgedClient.nick);
...
var loginToServer = function loginToServer(server) {
    ...
    if (!bridgedClient) {
        ...
        log.debug("Created new bot client for %s (disabled=%s): %s", server.domain, bridgedClient.disabled, bridgedClient._id);
    }
    var chansToJoin = [];
    return getChannelsToJoin(server).then(function (channels) {
        log.info("Bot connecting to %s (%s channels) => %s", server.domain, channels ? channels.length : "0", JSON.stringify(channels));
        ...
        log.error("Bot failed to connect to %s : %s - Retrying....", server.domain, JSON.stringify(err));
        log.logErr(err);
...
var getServerFromUserId = function getServerFromUserId(userId) {
    for (var i = 0; i < servers.length; i++) {
        var server = servers[i];
...
var userIdToServerNick = function userIdToServerNick(userId) {
