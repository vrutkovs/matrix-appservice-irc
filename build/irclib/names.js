 ...
 */
var generateIdentUsername = function generateIdentUsername(domain, userId) {
    // @foobar££stuff:domain.com  =>  foobar__stuff_domain_com
    var d = q.defer();
    var uname = sanitiseUsername(userId.substring(1));
    if (uname < module.exports.MAX_USER_NAME_LENGTH) {
        // bwahaha not likely.
        return uname;
    }
...
var checkQueue = function checkQueue() {
    if (!processing) {
        ...
        log.debug("Generating username for %s on %s (new queue length = %s)", matrixUser.userId, ircUser.server.domain, queue.length);
        generateIdentUsername(ircUser.server.domain, matrixUser.userId).then(function (uname) {
            ircUser.username = uname;
            // persist to db here before releasing the lock on this
            // request.
            return store.ircClients.set(matrixUser.userId, ircUser);
        }, function (e) {
            log.error("Failed to generate username for %s : %s", matrixUser.userId, e);
            defer.reject(e);
            processing = null;
            checkQueue();
        }).done(function () {
        ...
        log.debug("Already generating for %s on %s", processing.matrixUser.userId, processing.ircUser.server.domain);
    }
};
...
module.exports = {
     ...
     * }
     */
    getIrcNames: function getIrcNames(ircUser, matrixUser) {
        var d = q.defer();
        ...
        if (matrixUser) {
            ...
            if (ircUser.username) {
                log.debug("Using cached ident username %s for %s on %s", ircUser.username, matrixUser.userId, ircUser.server.domain);
                ...
                log.debug("Pushed username generation request for %s on %s to the queue...", matrixUser.userId, ircUser.server.domain);
                queueDefer.promise.done(function (uname) {
                    info.username = uname;
                    d.resolve(info);
                }, function (err) {
                    log.error("Failed to generate ident username for %s on %s : %s", matrixUser.userId, ircUser.server.domain, err);
                    d.reject(err);
                });
                checkQueue();
            }
        } else {
            info.username = sanitiseUsername(ircUser.username // the bridge won't have a mx user
            );
            info.realname = sanitiseRealname(info.username);
        }
        info.realname = info.realname.substring(0, module.exports.MAX_REAL_NAME_LENGTH);

        if (!generatingUsername) {
            info.username = info.username.substring(0, module.exports.MAX_USER_NAME_LENGTH);
            d.resolve(info);
        }
