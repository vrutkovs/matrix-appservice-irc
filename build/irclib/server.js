...
IrcServer.prototype.createBotIrcUser = function () {
    return new IrcUser(this, this.config.botConfig.nick, true, this.config.botConfig.password);
};
...
IrcServer.prototype.createsDynamicAliases = function () {
    return this.config.dynamicChannels.enabled && this.config.dynamicChannels.createAlias;
};
...
IrcServer.prototype.createsPublicAliases = function () {
    return this.createsDynamicAliases() && this.getJoinRule() === "public";
};
...
IrcServer.prototype.claimsUserId = function (userId) {
    ...
    var regex = templateToRegex(this.config.matrixClients.userTemplate, {
        "$SERVER": this.domain
    }, {
        "$NICK": "(.*)"
    }, ":.*");
...
IrcServer.prototype.claimsAlias = function (alias) {
    ...
    var regex = templateToRegex(this.config.dynamicChannels.aliasTemplate, {
        "$SERVER": this.domain
    }, {
        "$CHANNEL": "(.*)"
    }, ":.*");
...
IrcServer.prototype.getAliasRegex = function () {
    return templateToRegex(this.config.dynamicChannels.aliasTemplate, {
...
IrcServer.prototype.getUserRegex = function () {
    return templateToRegex(this.config.matrixClients.userTemplate, {
        "$SERVER": this.domain // find/replace $server
    }, {
...
function templateToRegex(template, literalVars, regexVars, suffix) {
    ...
    Object.keys(literalVars).forEach(function (varPlaceholder) {
        regex = regex.replace(new RegExp(escapeRegExp(varPlaceholder), 'g'), literalVars[varPlaceholder]);
    });
    ...
    Object.keys(regexVars).forEach(function (varPlaceholder) {
        ...
        new RegExp(escapeRegExp(escapeRegExp(varPlaceholder)), 'g'), regexVars[varPlaceholder]);
    });
