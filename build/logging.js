...
var makeTransports = function makeTransports() {
    var timestampFn = function timestampFn() {
        return new Date().toISOString().replace(/T/, ' ').replace(/\..+/, '');
    };
    var formatterFn = function formatterFn(opts) {
        return opts.timestamp() + ' ' + opts.level.toUpperCase() + ':' + (opts.meta && opts.meta.loggerName ? opts.meta.loggerName : "") + ' ' + (opts.meta && opts.meta.reqId ? "[" + opts.meta.reqId + "] " : "") + (opts.meta && opts.meta.dir ? opts.meta.dir : "") + (undefined !== opts.message ? opts.message : '');
    };
...
var createLogger = function createLogger(nameOfLogger) {
...
module.exports = {
    ...
    newRequestLogger: function newRequestLogger(baseLogger, requestId, isFromIrc) {
        var decorate = function decorate(fn, args) {
            var newArgs = [];
        ...
        return {
            debug: function debug() {
                decorate(baseLogger.debug, arguments);
            },
            info: function info() {
                decorate(baseLogger.info, arguments);
            },
            warn: function warn() {
                decorate(baseLogger.warn, arguments);
            },
            error: function error() {
                decorate(baseLogger.error, arguments);
            },
            log: function log() {
                decorate(baseLogger.log, arguments);
            }
        };
    },

    setUncaughtExceptionLogger: function setUncaughtExceptionLogger(exceptionLogger) {
