...
var createAction = function createAction(protocol, action, opts) {
    return extend({
        action: action,
        protocol: protocol
    }, opts);
...
var createIrcAction = function createIrcAction(action, opts) {
    return createAction("irc", action, opts);
};

var createMatrixAction = function createMatrixAction(action, opts) {
...
module.exports.irc = {
    ...
    createEmote: function createEmote(text) {
        return createIrcAction(ACTIONS.EMOTE, {
            text: text
        });
    },
    createNotice: function createNotice(notice) {
...
module.exports.matrix = {
    ...
    createAction: function createAction(event) {
        ...
        if (event.type === "m.room.message") {
            var fmtText = event.content.format === "org.matrix.custom.html" ? event.content.formatted_body : undefined;
            ...
            if (event.content.msgtype === "m.image" || event.content.msgtype === "m.file") {
                var fileSize = "";
                if (event.content.info && event.content.info.size && typeof event.content.info.size === "number") {
                    fileSize = " (" + Math.round(event.content.info.size / 1024) + "KB)";
                }
                body = matrixLib.decodeMxc(event.content.url) + " - " + event.content.body + fileSize;
            }
