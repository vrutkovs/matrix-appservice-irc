...
var doRequest = function doRequest(opts, callback) {
    var req = modelRequests.findRequest(opts._matrix_opts._reqId);
    var log = req ? req.log : logger;

    log.debug("%s %s %s Body: %s", opts.method, opts.uri, opts.qs.user_id ? "(" + opts.qs.user_id + ")" : "(AS)", opts.body ? JSON.stringify(opts.body).substring(0, 80) : "");
...
var logResponse = function logResponse(log, opts, err, response, body) {
    ...
    if (err) {
        log.error("%s %s %s HTTP %s Error: %s", opts.method, opts.uri, userId ? "(" + userId + ")" : "(AS)", httpCode, JSON.stringify(err));
        return;
    }
    if (httpCode >= 300 || httpCode < 200) {
        log.error("%s %s %s HTTP %s Error: %s", opts.method, opts.uri, userId ? "(" + userId + ")" : "(AS)", httpCode, JSON.stringify(body));
    } else {
