...
var mkUserId = function mkUserId(localpart) {
    return "@" + localpart + ":" + hsDomain;
};

var getAliasLocalpart = function getAliasLocalpart(alias) {
...
MatrixLib.prototype.addAlias = function (roomId, alias) {
    ...
    client.createAlias(alias, roomId).done(function (res) {
        defer.resolve(res);
    }, function (err) {
        defer.reject(err);
    });
 ...
 */
MatrixLib.prototype.createRoomWithAlias = function (alias, name, topic, joinRule, publishRoom) {
    ...
    return client.createRoom({
        ...
        if (joinRuleToSet) {
            self.log.info("Created room. Setting join_rules for %s to %s", alias, joinRuleToSet);
...
MatrixLib.prototype.setInviteOnly = function (room, isInviteOnly) {
    ...
    setJoinRule(client, room.roomId, joinRule).done(function () {
        ...
        defer.reject("Failed to set join_rules on room: %s", JSON.stringify(e));
    });
    return defer.promise;
};
...
MatrixLib.prototype.initialSync = function (userId) {
    var client = this._getClient(userId);
    return client._http.authedRequest(undefined, "GET", "/initialSync", { limit: 0 });
};
...
var getMatrixUser = function getMatrixUser(userLocalpart, displayName) {
    ...
    var createUserFn = function createUserFn() {
        client.register(userLocalpart).done(function (response) {
            // user was create successfully
            var newUser = new MatrixUser(response.user_id, displayName, true);
            ...
            userClient.setDisplayName(displayName).done(function () {
                ...
                store.users.set(newUser, userLocalpart, displayName, true)["finally"](function () {
                    defer.resolve(newUser);
                });
            }, function (e) {
                // non-fatal to not set the display name.
                // persist in db and return user regardless of store outcome
                store.users.set(newUser, userLocalpart, displayName, false)["finally"](function () {
                    defer.resolve(newUser);
                });
            });
        }, function (err) {
            if (err.errcode == "M_USER_IN_USE") {
                ...
                var newUser = new MatrixUser(mkUserId(userLocalpart), displayName, true);
                // persist in db and return user regardless of store outcome
                store.users.set(newUser, userLocalpart, displayName, false)["finally"](function () {
                    defer.resolve(newUser);
                });
