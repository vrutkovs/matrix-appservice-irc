...
MemberListSyncer.prototype.sync = Promise.coroutine(function*() {
    ...
    yield this._leaveIrcUsersFromRooms(rooms, server);
...
MemberListSyncer.prototype._leaveIrcUsersFromRooms = function(rooms, server) {
    /*
    REMOVE:
    find the IRC users on Matrix
        that are not present in the IRC membership list
        but are present in a channel bridged to the room in question.
    */

    // let virtualUsers =

    rooms = rooms.filter((r) => r.remoteJoinedUsers.length > 0);

    console.log(rooms);
    let roomIds = rooms.map((r) => r.id);
    this.ircBridge.getStore().getIrcChannelsForRoomIds(roomIds).then(
        (roomIdToIrcRooms) => {
            Object.keys(roomIdToIrcRooms).map((roomId) => {
                let room = rooms.find((r) => r.id === roomId);
                let ircRooms = roomIdToIrcRooms[roomId];

                // Call names once for each room, using the first remote user

                let frontierUserId = room.remoteJoinedUsers[0];

                console.log(frontierUserId);

                let bridgedClient = this.ircBridge.getIrcUserFromCache(server, frontierUserId);

                console.log(bridgedClient.nick + ' will get names');
            });
        }
    );
