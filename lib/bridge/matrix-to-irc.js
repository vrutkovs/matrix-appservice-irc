 ...
 */
var handleAdminRoomInvite = Promise.coroutine(function*(req, event, inviter, botUser) {
    ...
    yield req.mxLib.joinRoom(event.room_id, botUser);
 ...
 */
var handleInviteFromBot = Promise.coroutine(function*(req, event, invitedIrcUser) {
    ...
    let mxUser = yield createMatrixUserForIrcUser(invitedIrcUser, req);
    yield req.mxLib.joinRoom(event.room_id, mxUser);
});

var handleInviteFromUser = Promise.coroutine(function*(req, event, invitedIrcUser) {
    ...
    let invitedUser = yield createMatrixUserForIrcUser(invitedIrcUser, req);
    yield req.mxLib.joinRoom(event.room_id, invitedUser);
    req.log.info("Joined %s to room %s", invitedUser.userId, event.room_id);
    let isPmRoom = yield req.mxLib.isPmRoom(
    ...
    try {
        yield req.mxLib.sendAction(mxRoom, invitedUser, notice);
    }
    catch (err) {
        // ignore, we want to leave the room regardless.
    }
    yield req.mxLib.leaveRoom(invitedUser.userId, event.room_id);
});
...
var onAdminMessage = Promise.coroutine(function*(req, event, adminRoom) {
    ...
    let ircServer = ircLib.getServer(args[0]);
    if (ircServer) {
        ...
        else if (ircLib.getServers().length === 1) {
            ircServer = ircLib.getServers()[0];
        }
        else {
            let notice = actions.matrix.createNotice(
    ...
    if (cmd === "!nick") {
        ...
        if (matrixRooms.length > 0) {
            ...
            let promises = matrixRooms.map(function(room) {
                req.log.info(
                    "Inviting %s to room %s", event.user_id, room.roomId
                );
                return req.mxLib.invite(room, event.user_id);
            });
        ...
        let ircRoom = yield ircLib.trackChannel(ircServer, ircChannel);
 ...
 */
var _onInvite = Promise.coroutine(function*(req, event, inviter, invitee) {
    /*
    ...
    // invite yet!)
    processingInvitesForRooms[event.room_id + event.state_key] = req.getPromise();
    req.getPromise().finally(function() {
    ...
    try {
        let ircUser = yield ircLib.matrixToIrcUser(invitee);
        // the invitee does map to an IRC user: is the invite from the
        // bot?
        if (matrixLib.getAppServiceUserId() === event.user_id) {
            yield handleInviteFromBot(req, event, ircUser); // case [2]
        }
        else {
            yield handleInviteFromUser(req, event, ircUser); // case [1]
        }
    }
    catch (err) {
        ...
        if (matrixLib.getAppServiceUserId() === event.state_key) {
            // case [3]
            yield handleAdminRoomInvite(req, event, inviter, invitee);
        }
...
var _onJoin = Promise.coroutine(function*(req, event, user) {
    ...
    ircRooms.forEach(function(room) {
        ...
        promises.push(Promise.coroutine(function*() {
            let bridgedClient = yield req.ircLib.getBridgedClient(room.server, user.userId);
...
var _onLeave = Promise.coroutine(function*(req, event, user) {
    ...
    let clientList = ircLib.getBridgedClientsForUserId(user.userId);
    ...
    ircRooms.forEach((ircRoom) => {
        if (!ircRoom.server.shouldJoinChannelsIfNoUsers()) {
            var syncer = this.getMemberListSyncer(ircRoom.server.domain);
            if (syncer) {
                syncer.checkBotPartRoom(ircRoom, req);
            }
        }
    });
...
module.exports.onLeave = function(req, event, user) {
    return reqHandler(req, _onLeave(req, event, user));
};
...
module.exports.onAliasQuery = function(req, alias) {
    return reqHandler(req, _onAliasQuery(req, alias));
};

module.exports.onUserQuery = function(req, userId) {
