/* @flow */
"use strict";
import {IrcUser} from "../models/users";
...
var createMatrixUserForIrcUser = function(ircUser: IrcUser,
                                          req: Request): Promise<MatrixUser> {
    var defer = q.defer();
...
var handleInviteFromUser = function(event, req, invitedIrcUser) {
    ...
    // FIXME: Make invited user be passed in; setting null makes flow unhappy
    // because it doesn't know invitedUser = mxUser happens before accessing its
    // props.
    var invitedUser = {};
    createMatrixUserForIrcUser(invitedIrcUser, req).then(function(mxUser) {
        invitedUser = mxUser;
...
var onAdminMessage = function(event, req, adminRoom) {
    ...
    if (event.content.body.indexOf("!nick") === 0) {
        ...
        if (ircServer.claimsUserId(event.user_id)) {
            ...
            req.reject(requests.ERR_VIRTUAL_USER);
            return req.getPromise();
        }
        ...
        function(bridgedClient) {
            ...
            return req.mxLib.sendAction(
                adminRoom, botUser, actions.matrix.createNotice(response)
            );
        }, function(err) {
            if (err.stack) {
                log.logErr(err);
            ...
            return req.mxLib.sendAction(
                adminRoom, botUser, actions.matrix.createNotice(JSON.stringify(err))
            );
        }).done(req.sucFn, req.errFn);
    }
        ...
        function(matrixRooms) {
            if (matrixRooms.length > 0) {
                ...
                // FIXME: Refactor this so ircRoom is passed in
                var ircRoom = new IrcRoom(server, ircChannel);
                ircLib.trackChannel(server, ircChannel).then(function(room: IrcRoom) {
                    ircRoom = room;
                    // implied private
                    ...
                    return store.rooms.set(ircRoom, mxRoom, false);
                }).then(function() {
                    req.log.info(
 ...
 */
module.exports.onInvite = function(event: Object, inviter: MatrixUser,
                                   invitee: MatrixUser) {
    /*
...
module.exports.onJoin = function(event: Object, user: MatrixUser): QPromise {
    var req = requests.newRequest(false);
...
module.exports.onLeave = function(event: Object, user: MatrixUser): Promise {
    var req = requests.newRequest(false);
 ...
 * @return {Promise} which is resolved/rejected when the request finishes.
 */
module.exports.onMessage = function(event: Object, existingRequest: ?Request): Promise {
    /*
     * Valid message flows:
    ...
    var mxAction = actions.matrix.createAction(event);
    if (!mxAction) {
        req.errFn("Cannot create action for event");
        return req.getPromise();
    }
    var ircAction = actions.toIrc(mxAction);
    store.rooms.getIrcChannelsForRoomId(event.room_id).done(
    function(ircRooms) {
        if (ircRooms.length === 0) {
 ...
 * @return {Promise} which is resolved/rejected when the request finishes.
 */
module.exports.onAliasQuery = function(roomAlias: string): Promise {
    /* type {Request} */
    var req = requests.newRequest(false);
    ...
    if (!channelInfo.channel) {
        req.errFn("Unknown alias: " + roomAlias);  // bad alias
        return req.getPromise();
    }
    ...
    store.rooms.getMatrixRoomsForChannel(
        ...
        if (matrixRooms.length === 0) {
            ...
            ircLib.trackChannel(channelInfo.server, channelInfo.channel).then(
                ...
                store.rooms.set(ircRoom, matrixRoom, false).done(function() {
                    req.sucFn();
                }, function(e) {
                    req.errFn(e);
                });
 ...
 * @return {Promise} which is resolved/rejected when the request finishes.
 */
module.exports.onUserQuery = function(userId: string): Promise {
    /* type {Request} */
    var req = requests.newRequest(false);
