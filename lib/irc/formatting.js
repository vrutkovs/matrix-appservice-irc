...
module.exports.ircToHtml = function(text) {
    ...
    if (colorRegex.test(text)) {
        ...
        while (groups) {
            ...
            var bgColor = colorCodesToHtmlNames[groups[2]];
            ...
            if (bgColor) {
                textCss['background-color'] = bgColor;
            }

            if (fontColor) {
                textCss['color'] = fontColor;
            }

            // Generate inline CSS
            var cssString = "";
            for (var i in textCss) {
                cssString = cssString + i + ":" + textCss[i] + ";";
            }
            ...
            if (bgColor || fontColor) {
                text = text.replace(
                    groups[0], '<font color="green">green</font>'+
                    '<b>bold</b>'+
                    '<a href="localhost">link</a>'+
                    '<span style="color:red">SPAN</span>'+
                    '<div style="color:red">div</div>'+
                    '<table><tr><td bgcolor="pink">lol</td></tr>'
