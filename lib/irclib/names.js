...
var generateIdentUsername = function(domain, userId) {
    ...
    function loop() {
        ...
        store.ircClients.getByUsername(domain, uname).done(function(ircClient) {
            if (ircClient && ircClient.userId !== userId) { // occupied username!
                var usr = ircClient.ircUser;
...
var checkQueue = function() {
    if (!processing) {
        ...
        log.debug(
            ...
            matrixUser.userId, ircUser.server.domain, queue.length
        );
        ...
        log.debug(
            ...
            processing.matrixUser.userId, processing.ircUser.server.domain
        );
    }
};
        ...
        if (matrixUser) {
            ...
            if (ircUser.username) {
                ...
                log.debug(
                    ...
                    matrixUser.userId, ircUser.server.domain
                );
                queueDefer.promise.done(function(uname) {
                    ...
                    log.error(
                        ...
                        matrixUser.userId, ircUser.server.domain, err
                    );
                    d.reject(err);
                });
                checkQueue();
