...
var insert = function(database, d, objects) {
...
var update = function(database, d, query, updateVals) {
    database.update(query, updateVals, {upsert: false}, function(err, result) {
        callbackFn(d, err, result);
    });
};
var del = function(database, d, query) {
...
module.exports.rooms = {
     ...
     * @return {Promise}
     */
    set: function(ircRoom: IrcRoom, matrixRoom: MatrixRoom, fromConfig: boolean): Promise {
        var d = q.defer();
    ...
    getIrcChannelsForRoomId: function(roomId: string): Promise {
        ...
        select(getCollection("rooms"), d, {
            ...
            for (var i = 0; i < docs.length; i++) {
                var doc = docs[i];
                if (!doc.irc_addr || !serverMappings[doc.irc_addr]) {
                    continue;
                }
                var server = serverMappings[doc.irc_addr];
...
module.exports.ircClients = {
    ...
    update: function(userId: string, domain: string, key: string,
                     newVal: string): Promise {
        var d = q.defer();
        log.info(
    ...
    getByUsername: function(domain: string,
                            username: string): Promise<?{ircUser: IrcUser, userId: string}> {
        var d = q.defer();
        select(getCollection("irc_clients"), d, {
            domain: domain,
            username: username
